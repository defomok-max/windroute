...
//   1. LOG_DIR env (explicit override for containers/CI)
//   2. config.logsDir (%USERPROFILE%\.windbu\logs on Windows by default)
//   3. OS temp (windbu-logs) — last resort so we never crash on startup
function resolveLogDir() {
  const candidates = [];
  if (process.env.LOG_DIR) candidates.push(process.env.LOG_DIR);
  if (config?.logsDir) candidates.push(config.logsDir);
  candidates.push(join(tmpdir(), 'windbu-logs'));
